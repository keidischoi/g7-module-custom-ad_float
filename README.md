...
마이그레이션은 없습니다. 관리자에서 「이미지 업로드」를 눌러도 업로드 박스가 안 나오던 문제와, 예약창 삭제가 DB에 반영되지 않던 문제를 고칩니다. **관리자 페이지는 캐시 삭제 후 하드 리프레시** 하세요. 프론트 `ad-float.js` 캐시 쿼리는 `?v=0.1.25` 입니다.
...
마이그레이션은 없습니다. 관리자 다크 테마에서 「캐러셀 옵션」 흰 박스가 빠집니다. X 버튼은 기본 설정만 따르고 예약이 덮어쓰지 않습니다. 「예약 추가」가 먹통이던 표현식을 짧게 바꿨습니다. 광고 목록 행 간격과 광고 등록/새로고침 여백을 줄였습니다. 광고 등록 업로드는 0.1.25에서 파일 입력 박스로 바꿨습니다. 프론트 `ad-float.js` 캐시 쿼리는 `?v=0.1.21` 입니다.
