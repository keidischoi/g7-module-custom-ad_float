...
마이그레이션은 없습니다. 관리자 다크 테마에서 「캐러셀 옵션」 흰 박스가 빠집니다. X 버튼은 기본 설정만 따르고 예약이 덮어쓰지 않습니다. 「예약 추가」가 먹통이던 표현식을 짧게 바꿨습니다. 광고 등록은 **업로드 박스(FileUploader)** 를 씁니다. 프론트 `ad-float.js` 캐시 쿼리는 `?v=0.1.21` 입니다.
