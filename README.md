...
- 관리자 `/admin/ad-float`: **기본 설정** → **광고창** → **광고 목록** → **날짜 및 시간 예약**. 「통계 보기」로 `/admin/ad-float/stats` (노출·클릭, 광고별 × 페이지별)
- **광고창**을 여러 장 두면 왼쪽+오른쪽처럼 동시에 여러 위치에 표시됩니다. 비어 있으면 기존처럼 기본 설정의 위치 하나만 씁니다.
...
- **예약 사용 OFF** (기본값): 사용 중인 광고를 대상으로 합니다.
- **예약 사용 ON**: 위에서부터 **첫 번째로 일치하는** 예약만 적용합니다.
  ...
  - 일치하면 그 행이 선택한 광고(`item_ids`, 비우면 사용 중 광고 전부)만 허용합니다.
  - 일치하는 행이 없으면 광고를 숨깁니다.
- **광고창(`placements`) 비어 있음** (기본값): 위 규칙 + **기본 설정**(예약 ON이면 일치 행의 위치·크기 등)으로 창 하나.
- **광고창이 1장 이상**: 켜진 창마다 루트를 따로 붙입니다. 예약은 *어떤 광고가 허용되는지*만 정하고, 각 창의 `position` 등은 창 설정을 따릅니다. 창의 `item_ids`가 비면 예약이 허용한 광고 전부, 있으면 그 교집합. 캐러셀 결합은 창별 목록 안에서 그대로 동작합니다.
...
방문자가 플로팅 광고 **X**를 누르면 `close_cookie_key`(기본 `g7_custom_ad_float_closed`)로 **쿠키 + localStorage**에 저장되어, 그 브라우저에서는 다시 보이지 않습니다. 광고창이 여러 장이면 키 뒤에 `_{창 id}`가 붙어서, 한쪽만 닫아도 다른 창은 남습니다. (광고창을 쓰지 않는 기본 창은 예전처럼 접미사 없는 키입니다.)
...
`custom_ad_float_settings.placements` JSON이 추가됩니다 (마이그레이션 `2026_09_15_000008`). 컬럼이 없거나 배열이 비어 있으면 기존 단일 위치 동작입니다. 관리자에서 「광고창」을 2장 이상 추가하면 홈에 동시에 표시됩니다. 프론트 `ad-float.js` 캐시 쿼리는 `?v=0.1.18` 입니다.
