...
- 위치·캐러셀·모바일·닫기 쿠키 등 기본 표시 옵션. 클릭 URL은 **현재 창 / 새 창 / 팝업 모달**. 관리자 「닫힘 상태 초기화」로 방문자가 닫았던 광고를 다시 노출
...
마이그레이션은 없습니다. 「이미지 업로드」에서 파일을 고른 뒤 **광고 등록**이 422 `이미지 파일을 선택해 주세요`로 실패하던 원인을 고칩니다. FileUploader 박스는 그대로이고, 컴포넌트가 직접 `file`을 올립니다. 등록 실패 시 에러 토스트가 본문을 보여 줍니다. **관리자 페이지는 캐시 삭제 후 하드 리프레시** 하세요. 프론트 `ad-float.js` 캐시 쿼리는 `?v=0.1.28` 입니다.
...
마이그레이션은 없습니다. 「이미지 업로드」에서 파일을 고른 뒤 **광고 등록**이 DB에 저장되지 않던 문제를 고칩니다. FileUploader 박스는 그대로입니다. **관리자 페이지는 캐시 삭제 후 하드 리프레시** 하세요. 프론트 `ad-float.js` 캐시 쿼리는 `?v=0.1.27` 입니다.
...
마이그레이션은 없습니다. 「이미지 업로드」에 G7 **FileUploader** 업로더 박스를 다시 씁니다. 파일 선택 후 **광고 등록**이 실제로 저장됩니다. **관리자 페이지는 캐시 삭제 후 하드 리프레시** 하세요. 프론트 `ad-float.js` 캐시 쿼리는 `?v=0.1.26` 입니다.
...
마이그레이션은 없습니다. 관리자에서 「이미지 업로드」를 눌러도 업로드 박스가 안 나오던 문제와, 예약창 삭제가 DB에 반영되지 않던 문제를 고칩니다. 업로드 UI는 0.1.26에서 FileUploader 박스로 되돌렸습니다. **관리자 페이지는 캐시 삭제 후 하드 리프레시** 하세요. 프론트 `ad-float.js` 캐시 쿼리는 `?v=0.1.25` 입니다.
...
마이그레이션은 없습니다. 팝업 모달 디자인을 다듬었습니다 (블러 배경, 라운드 패널, 로딩 쉬머, 제목+× 헤더). 프론트 `ad-float.js` 캐시 쿼리는 `?v=0.1.24` 입니다.
...
`link_open_mode` 컬럼이 생깁니다 (`open_new_tab` 값으로 채움). 관리자 기본 설정에서 링크를 현재 창 / 새 창 / 팝업 모달로 고릅니다. 프론트 `ad-float.js` 캐시 쿼리는 `?v=0.1.23` 입니다.
...
마이그레이션은 없습니다. 관리자 다크 테마에서 「캐러셀 옵션」 흰 박스가 빠집니다. X 버튼은 기본 설정만 따르고 예약이 덮어쓰지 않습니다. 「예약 추가」가 먹통이던 표현식을 짧게 바꿨습니다. 광고 목록 행 간격과 광고 등록/새로고침 여백을 줄였습니다. 광고 등록 업로드는 0.1.21에서 FileUploader를 썼고, 0.1.25에서 파일 입력으로 바꿨다가 **0.1.26에서 FileUploader 박스로 복구**했습니다. 프론트 `ad-float.js` 캐시 쿼리는 `?v=0.1.21` 입니다.
